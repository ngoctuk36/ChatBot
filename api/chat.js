...
function isWebsiteHelpQuestion(message) {
  return WEBSITE_HELP_RE.test(message)
}

function isLikelyVagueWebsiteQuestion(message) {
  return VAGUE_WEBSITE_RE.test(message)
}

function buildMessages(message, history, websiteContext) {
  return [
...
async function callGroq(groqClient, messages) {
...
export default async function handler(req, res) {
  ...
  if (message.length > 5000) {
    return json(res, 400, { error: 'Câu hỏi quá dài. Vui lòng rút ngắn dưới 5000 ký tự.' })
  }

  if (isWebsiteHelpQuestion(message)) {
    return json(res, 200, { reply: WEBSITE_HELP_ANSWER })
  }

  if (isLikelyVagueWebsiteQuestion(message)) {
    return json(res, 200, { reply: WEBSITE_VAGUE_ANSWER })
  }
  ...
  try {
    ...
    let chatCompletion
    try {
      chatCompletion = await callGroq(groqClient, buildMessages(message, history, websiteContext))
    } catch (error) {
      ...
      // Fallback một lần: giữ câu hỏi hiện tại, nhưng cắt context mạnh hơn để tránh vượt 8K TPM.
      const fallbackHistory = normalizeHistory(data.history, 2200)
      const fallbackWebsite = websiteContext.slice(0, 2200)
      chatCompletion = await callGroq(groqClient, buildMessages(message, fallbackHistory, fallbackWebsite))
    }
